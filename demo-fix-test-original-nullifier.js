...
async function runDemoFix() {
  // Use the final test input with original nullifier from the CLI demo
  const inputPath = path.resolve('./test-vector-fix/final-test-input-with-original-nullifier.json');
  ...
  try {
    ...
    // --- Export ---
    const outDir = path.resolve('./demo-fix-output-original-nullifier');
