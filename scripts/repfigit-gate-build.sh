...
if [ -z "${REPO_DIR:-}" ]; then
    echo "repfigit-gate-build: REPO_DIR not provided (argument 1 or env)" >&2
    echo "Silent no-op (expected behavior for no-work tick)" >&2
    exit 0
fi

# Execute the actual gate script with the repo directory
exec "$SCRIPT_DIR/repfigit-build-gate.sh" \
    "$REPO_DIR" "${TEAM_KEY:-AIF}" "${REPO_PROJECT:-}" "${TIER_FILTER:-all}"