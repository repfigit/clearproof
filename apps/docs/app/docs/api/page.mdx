...
All endpoints except `/health` and `/auth/*` require authentication. Local development defaults to API-key mode:
...
Protected requests then include:

```bash
-H "X-API-Key: dev-api-key"
```

Production deployments should use `AUTH_MODE=jwt` or `AUTH_MODE=siwe`.
