...
The source API provides a five-minute EOA signing challenge and a 24-hour,
revocable attestation bound to an enrolled credential and authenticated tenant.
Use `/wallet/ownership/challenge`, `/wallet/ownership/verify`,
`/wallet/ownership/attestations/{id}`, `/wallet/ownership/revoke` and
`/wallet/ownership/credential`. Existing credential/proof formats are unchanged.
The extension circuit is staged; existing deployed verifiers do not accept it.
See the [wallet ownership guide](https://github.com/repfigit/clearproof/blob/main/docs/internal/WALLET_OWNERSHIP.md)
for requests, signing, authorization, expiry and replay behavior.
