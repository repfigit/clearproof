...
**Pilot-stage, pre-production software.** Published npm packages: **0.5.0** (`@clearproof/circuits` 0.3.0). Main GitHub repository: **public**.

The implementation below is merged into `main` (PR #27, September 7, 2026) and has been extended since. It is published in the 0.5.0 npm packages. It is not in the historical Sepolia deployments. Local acceptance evidence does not make it production software.

## Available components and limits

| Area | Current development status |
| --- | --- |
| Public installation | `@clearproof/proof`, `cli`, `content` and `contracts` 0.5.0 install from npm (verified September 26, 2026); `proof`, `cli` and `contracts` carry signed build provenance |
| Proof components | Credential-bound Groth16 `pilot-transfer-v3` statement (tree depths 32/20/20: about 4.3 billion credentials per issuer, a million issuers, a million sanctioned addresses), holder-secret and issuer membership checks, sanctions screening of both wallets, exact transfer projection and valuation arithmetic, tested with real development proofs in CI |
...
| Deployment and assurance | Historical Sepolia deployments of the legacy contracts only; the pilot verifier and registry are not deployed. No completed independent circuit/contract audit or production trusted setup |

The clean-checkout workflow installs locked dependencies, builds source workspaces, generates explicitly unapproved development artifacts and exercises real PostgreSQL, CLI/API and local EVM paths. Retained encrypted exports can be reviewed after the services stop. These are controlled synthetic tests with configured authorities, not proof of live source truth or legal compliance. The install-and-build portion of this workflow was re-verified from a clean public clone of `main` on September 15, 2026 at commit `1c8d385` (locked workspace install, workspace build, built-CLI `--help`, Python environment sync, Python unit suite: 1,923 passed, 4 skipped).
...
Customer-dependent issuer/wallet support, live provider integration, ongoing re-screening feeds, commercial validation and independently reviewed production operations remain separate follow-on gates. The [repository is now public](https://github.com/repfigit/clearproof). The local pilot is merged into `main` through PR #27; updated package distribution is not yet available.
