...
**Pilot-stage, pre-production software.** Published npm packages: **0.6.0**. Main GitHub repository: **public**.

The implementation below is merged into `main` (PR #27, September 7, 2026) and has been extended since. It is published in the npm packages (0.5.0 and later). It is not in the historical Sepolia deployments. Local acceptance evidence does not make it production software.

## Available components and limits

| Area | Current development status |
| --- | --- |
| Public installation | All five `@clearproof` packages are at 0.6.0 on npm and publish from GitHub Actions with signed build provenance; `circuits` is source only |
