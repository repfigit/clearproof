...
[Apache-2.0](LICENSE) for all original clearproof code, per [REUSE.toml](REUSE.toml) (SPDX metadata, verified in CI).

One exception: [`packages/contracts/contracts/Groth16Verifier.sol`](packages/contracts/contracts/Groth16Verifier.sol) is generated by snarkjs from a GPL-3.0 template (Copyright 2021 0KIMS association) and retains its per-file `SPDX-License-Identifier: GPL-3.0` header. License texts live in [`LICENSES/`](LICENSES/).
