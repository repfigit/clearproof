...
Status: development record profile used by the local pilot. The legacy proof API remains a separate interface. A record digest does not prove credential issuance, authenticate a holder, establish quote truth or authorize a transfer. The [current statement service](../docs/internal/PILOT_CURRENT_STATEMENT.md) and [atomic authorization service](../docs/internal/PILOT_AUTHORIZATION.md) supply the separate trust and transactional boundaries; this specification describes the records they bind.

## Records and authority

`Transfer` contains private transfer facts: tenant and transfer IDs, a 32-byte nonce, exact originator/beneficiary wallets and VASP/self-hosted roles, asset identity/catalog digest, quantity, valuation, jurisdiction, policy digest and validity interval. Serialized transfers belong inside encrypted envelopes, not logs, public proof signals or plaintext storage. Transfer creators should supply a cryptographically random nonce; the record validator checks its representation, not its randomness. Deterministic nonces in fixtures are synthetic only.
...
Domains are `clearproof/transfer/v1`, `clearproof/verification-context/v1`, `clearproof/evidence-receipt/v1` and `clearproof/asset-registry/v1`. Results are lowercase 64-character hex strings. No hash is silently reduced into the SNARK scalar field. The [pilot-transfer-v2 statement](pilot-transfer-v2.md) describes the separate 48-field circuit-facing projection and credential-bound outer commitment. Hashing JSON alone does not solve that binding; current inspection reconstructs it using independently authenticated inputs.
