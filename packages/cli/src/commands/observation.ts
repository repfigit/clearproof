...
import { createObservation, readObservation, reportObservationCohort } from '@clearproof/proof';
...
for (const operation of ['create', 'read', 'report'] as const) {
  observationCommand.command(operation)
    ...
    .action(async (options: { apiUrl: string }) => {
      try {
        reportEndpoint(options.apiUrl, operation === 'create' ? '/pilot/proof/observe' : operation === 'read' ? '/pilot/proof/observations/read' : '/pilot/proof/observations/report');
        const token = process.env.CLEARPROOF_API_TOKEN ?? '';
        if (!token) throw new Error('API token required');
        const report = await (operation === 'create' ? createObservation : operation === 'read' ? readObservation : reportObservationCohort)(
