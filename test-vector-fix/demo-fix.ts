...
export const demoFixCommand = new Command('demo-fix')
  .description('Run a demo to test the fixed wallet ownership verification')
  .option('--export <dir>', 'Directory to export test vector')
  .action(async (opts: { export?: string }) => {
    const inputPath = path.resolve('./test-vector-fix/input.json');
